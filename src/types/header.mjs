...
function header1Element(first, last, content) {
    ...
    return `
    <h1 id="${id}" class="markdown header1" >${content}</h1>
    `
}
...
function header3Element(first, last, content) {
    const id = idfy(content)
    return `
    <h3 id="${id}" class="markdown header3" >${content}</h3>
    `
}
function header4Element(first, last, content) {
    const id = idfy(content)
    return `
    <h4 id="${id}" class="markdown header4" >${content}</h4>
    `
}
function header5Element(first, last, content) {
    const id = idfy(content)
    return `
    <h5 id="${id}" class="markdown header5" >${content}</h5>
    `
}
function header6Element(first, last, content) {
    const id = idfy(content)
    return `
    <h6 id="${id}" class="markdown header6" >${content}</h6>
    `
}
